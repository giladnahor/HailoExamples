...
DETECTION_HEF_PATH=f'{RESOURCES_DIR}/yolov5s_personface.hef'
...
def get_decode_element():
    decode_element=f'decodebin ! {get_queue(3)} ! videoscale ! {get_queue(3)} ! videoconvert qos=false ! video/x-raw, format=RGB, width=1920, height=1080'
...
def create_sources(start=0, end=8, src_type='test'):
    ...
    for index in range(start, end):
        index_mod = index % 8
        if (src_type == 'file'):
            sources += create_file_src(index, uri=f'file://{RESOURCES_DIR}/video{index_mod}.mp4') 
        else:
            if (src_type == 'rtsp'):
        ...
        # # fake sink for all
        # sources += f'fakesink name=sink_{index} sync=false async=false '
        ...
        #             {get_queue(8)} ! \
        #             hailopython name=hailopython_camera_{index} qos=false module={COMMON_DIR}/send_zmq.py function=run_camera ! \
        #             {get_scaler()} ! \
        #             comp.sink_{index} '
    return sources
...
def init_sub_piplines():
    global DETECTOR_PIPELINE, ATTR_PIPELINE, COMP_DISP_PIPELINE, DISPLAY_PIPELINE, PERSON_FACE_DETECTION_PIPELINE
...
def build_pipeline(src_type='test'):
    init_sub_piplines()
    sources = create_sources(0, 8, src_type=src_type)
    
    pipeline = ''
    pipeline += f'{sources}'
    pipeline += f'hailoroundrobin name=roundrobin funnel-mode=false ! '
    # pipeline += f'funnel name=roundrobin ! '
    pipeline += get_queue(60, "pre_detector_pipe_q") + ' ! '
    pipeline += f'{PERSON_FACE_DETECTION_PIPELINE} ! '
    pipeline += f'videoconvert name=pre_send_zmq qos=false ! video/x-raw,format=I420 ! '
    pipeline += f'hailopython name=hailopython_camera qos=false module={COMMON_DIR}/send_zmq.py function=run_camera ! '
    # pipeline += 'fakesink name=hailo_display sync=false'
    pipeline += f'{DISPLAY_PIPELINE} '
...
def timeout_cb(loop, pdata):
    ...
    if  pdata.buffer_last_time is not None:
        cur_time = time.time()
        if cur_time - pdata.buffer_last_time > 1:
            print('Timeout killing pipeline')
            loop.quit()
            return False
    return True

def fps_cb(loop, pdata):
    framse_per_sec = pdata.buffer_count - pdata.buffer_count_last
    pdata.buffer_count_last = pdata.buffer_count
    print(f'FPS: {framse_per_sec}')
    return True
...
def main(src_type='test'):
    
    # GObject.threads_init()
    Gst.init(None)

    pipeline = build_pipeline(src_type=src_type)
    print(pipeline)
    ...
    GLib.timeout_add_seconds(1, fps_cb, loop, pdata)
    ...
    video_q_elem = pipe.get_by_name('hailo_display')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        src_type = sys.argv[1]
        if src_type not in ['rtsp', 'test', 'file']:
            print(f'Invalid source type {src_type} must be one of rtsp, test, file')
            sys.exit(1)
    if len(sys.argv) > 2:
    ...
    if python_on_top:
        sys.exit(main(src_type))
    else:
        pipeline = build_pipeline(src_type)
        print("-------------Running on shell--------------")
        cmd = f'gst-top-1.0 gst-launch-1.0 --no-position {pipeline} -v | grep hailo_display'
        print(cmd)
        os.system(cmd)
